...
const Card = ({ children, className = "" }) => (
  <div className={`relative group ${className}`}>
    <div className="absolute -inset-0.5 bg-gradient-to-r from-red-600 to-orange-500 rounded-2xl blur opacity-10 group-hover:opacity-20 transition duration-500 pointer-events-none" />
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start sm:items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          <div className="relative">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-red-600 to-orange-500 rounded-xl blur opacity-50 pointer-events-none" />
          ...
          {[
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 sm:px-4 py-1.5 rounded-lg text-xs sm:text-sm transition-all duration-200 ${
                filter === tab.value
                  ? "bg-gradient-to-r from-red-500/25 to-orange-500/20 border border-red-500/35 text-white font-medium"
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          { label: "Total", value: comments.length, color: "text-red-400" },
          { label: "Pinned", value: pinnedCount, color: "text-orange-400" },
          {
            label: "Unpinned",
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-10 py-2.5 text-gray-200 placeholder-gray-600 text-sm outline-none focus:border-red-500/50 focus:ring-1 focus:ring-red-500/20 transition-all"
        />
        {search && (
          <button
            onClick={() => setSearch("")}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <div className="w-7 h-7 border-2 border-white/10 border-t-red-500 rounded-full animate-spin" />
        </div>
      ) : paginated.length === 0 ? (
        <Card>
        ...
        <div className="space-y-2.5">
          {paginated.map((comment) => (
            <div key={comment.id} className="relative group">
              {comment.is_pinned && (
                <div className="absolute -inset-0.5 bg-gradient-to-r from-red-600 to-orange-500 rounded-2xl blur opacity-15 pointer-events-none" />
              )}
              <div
              ...
              >
                <div className="flex items-start gap-3 sm:gap-4">
                  {/* Avatar */}
                  <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-full bg-gradient-to-br from-red-500/20 to-orange-500/20 border border-white/10 flex items-center justify-center shrink-0">
                    <img
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg border transition-all duration-200 ${
                        comment.is_pinned
                          ...
                          : "border-white/10 text-gray-500 hover:text-red-400 hover:border-red-500/25"
                      }`}
                    >
                      {comment.is_pinned ? (
